    ...
    <Layout>
      ...
      <div className="max-w-6xl mx-auto px-8 py-16 flex space-x-24">
        <section className="flex-[2] space-y-8">
          <h2 className="text-cyan-500 uppercase mb-8 tracking-widest font-bold">
            {filterTag}
          </h2>
          {pagesByTag?.map((post) => (
            <PostListItem
              ...
              {popularPosts?.map((post) => (
                <li key={post.id}>
                  <Link href={`/blog/posts/${post.slug}`}>
                    <a className="!text-gray-200 hover:text-cyan-600 font-bold text-lg leading-10 transition-colors">
                      {post.name}
                    </a>
                  </Link>
                </li>
